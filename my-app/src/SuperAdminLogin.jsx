  ...
  return (
    <div className="min-h-screen bg-black flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="mx-auto w-14 h-14 md:w-16 md:h-16 bg-gradient-to-br from-red-500 to-orange-600 rounded-2xl flex items-center justify-center shadow-2xl shadow-red-500/20 mb-6">
          <ShieldCheck className="text-white w-7 h-7 md:w-8 md:h-8" />
        </div>
        <h2 className="text-center text-2xl md:text-3xl font-serif font-black italic text-white tracking-tight">Master Control</h2>
        <p className="mt-2 text-center text-xs md:text-sm font-bold text-slate-400 uppercase tracking-[0.2em]">
      ...
      <div className="mt-8 sm:mx-auto w-full sm:max-w-md relative z-10">
        <div className="bg-slate-900/80 backdrop-blur-xl py-8 px-6 shadow-2xl rounded-2xl md:rounded-[2rem] sm:px-10 border border-slate-800 transition-all duration-500">
          ...
          {/* --- LOGIN VIEW --- */}
          {view === 'login' && (
            <form className="space-y-5 md:space-y-6 animate-in fade-in slide-in-from-bottom-4" onSubmit={handleLogin}>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-black/50 border border-slate-800 rounded-xl py-4 pl-12 pr-4 font-bold text-white outline-none focus:ring-2 focus:ring-red-500 transition-all placeholder:text-slate-700 text-sm md:text-base" placeholder="e.g. founder@gourmet.com" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-black/50 border border-slate-800 rounded-xl py-4 pl-12 pr-4 font-bold text-white outline-none focus:ring-2 focus:ring-red-500 transition-all placeholder:text-slate-700 text-sm md:text-base" placeholder="••••••••" />
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-center pt-4 border-t border-slate-800 gap-3 sm:gap-0">
          ...
          {/* ... keeping other views structurally intact, mainly applying px-6 to the wrapper ... */}
          {/* --- UPDATE EMAIL VIEW (OTP FLOW) --- */}
          {view === 'update_email' && (
            <div className="space-y-5 md:space-y-6 animate-in fade-in slide-in-from-left-4">
              {!otpSent ? (
                <form onSubmit={handleSendOtp} className="space-y-5 md:space-y-6">
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="text" required value={recoveryKey} onChange={(e) => setRecoveryKey(e.target.value)} className="w-full bg-black/50 border border-slate-800 rounded-xl py-4 pl-12 pr-4 font-bold text-orange-500 outline-none focus:ring-2 focus:ring-red-500 uppercase text-sm md:text-base" placeholder="ENTER SECRET KEY" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="email" required value={newEmail} onChange={(e) => setNewEmail(e.target.value)} className="w-full bg-black/50 border border-slate-800 rounded-xl py-4 pl-12 pr-4 font-bold text-white outline-none focus:ring-2 focus:ring-red-500 text-sm md:text-base" placeholder="your.name@domain.com" />
                    </div>
                  </div>
                ...
                <form onSubmit={handleVerifyAndUpdateEmail} className="space-y-5 md:space-y-6">
                  <div>
                    ...
                    <div className="relative mt-4">
                      ...
                      <input type="text" required maxLength="6" value={enteredOtp} onChange={(e) => setEnteredOtp(e.target.value)} className="w-full bg-black/50 border border-slate-800 rounded-xl py-4 pl-12 pr-4 font-black text-2xl text-center text-white outline-none focus:ring-2 focus:ring-red-500 tracking-[0.3em] md:tracking-[0.5em]" placeholder="000000" />
                    </div>
                  </div>
                  <div className="flex gap-3">
          ...
          {/* --- PASSWORD RESET VIEW --- */}
          {view === 'reset_pass' && (
            <form className="space-y-5 md:space-y-6 animate-in fade-in slide-in-from-right-4" onSubmit={handlePasswordReset}>
              <div>
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Secret Recovery Key</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500"><LockKeyhole size={18} /></div>
              ...
              <div>
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500"><KeyRound size={18} /></div>
                  <input type="password" required value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full bg-black/50 border border-slate-800 rounded-xl py-4 pl-12 pr-4 font-bold text-white outline-none focus:ring-2 focus:ring-red-500 text-sm md:text-base" placeholder="Create new password" />
                </div>
              </div>
              <div className="flex gap-3">
                <button type="button" onClick={resetToLogin} className="w-1/4 bg-slate-800 text-white py-4 rounded-xl flex justify-center items-center hover:bg-slate-700 transition-all"><ArrowLeft size={18} /></button>
